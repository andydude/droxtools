...
constant $BI_STAG = '<drox:binit>';
constant $BI_ETAG = '</drox:binit>';
constant $BL_STAG = '<m:bind>';
constant $BL_ETAG = '</m:bind>';
constant $BT_STAG = '<drox:btype>';
constant $BT_ETAG = '</drox:btype>';
constant $BV_STAG = '<m:bvar>';
constant $BV_ETAG = '</m:bvar>';
constant $RT_STAG = '<drox:rt>';
constant $RT_ETAG = '</drox:rt>';
constant $MD_STAG = '<drox:mods>';
constant $MD_ETAG = '</drox:mods>';
constant $SM_STAG = '<m:semantics>';
constant $SM_ETAG = '</m:semantics>';
...
method string-constant($/) {
    my $value = (map {$_.ast}, @<string-literal>).join;
    make $value;
}
...
method multiplicative-operator:sym<*>($/) {
    make '<m:csymbol cd="arith1">times</m:csymbol>';
}

method multiplicative-operator:sym</>($/) {
    make '<m:csymbol cd="arith1">divide</m:csymbol>';
}

method multiplicative-operator:sym<%>($/) {
    make '<m:csymbol cd="arith1">remainder</m:csymbol>';
}
...
method additive-operator:sym<+>($/) {
    make '<m:csymbol cd="arith1">plus</m:csymbol>';
}

method additive-operator:sym<->($/) {
    make '<m:csymbol cd="arith1">minus</m:csymbol>';
}
...
method shift-operator:sym«<<»($/) {
    make '<m:csymbol cd="bitwise1">shift_left</m:csymbol>';
}

method shift-operator:sym«>>»($/) {
    make '<m:csymbol cd="bitwise1">shift_right</m:csymbol>';
}
...
method relational-operator:sym«<»($/) {
    make '<m:csymbol cd="relation1">lt</m:csymbol>';
}

method relational-operator:sym«>»($/) {
    make '<m:csymbol cd="relation1">gt</m:csymbol>';
}

method relational-operator:sym«<=»($/) {
    make '<m:csymbol cd="relation1">leq</m:csymbol>';
}

method relational-operator:sym«>=»($/) {
    make '<m:csymbol cd="relation1">geq</m:csymbol>';
}
...
method equality-operator:sym<==>($/) {
    make '<m:csymbol cd="relation1">eq</m:csymbol>';
}

method equality-operator:sym<!=>($/) {
    make '<m:csymbol cd="relation1">neq</m:csymbol>';
}
...
method and-operator:sym<&>($/) {
    make '<m:csymbol cd="bitwise1">and</m:csymbol>';
}
...
method exclusive-or-operator:sym<^>($/) {
    make '<m:csymbol cd="bitwise1">xor</m:csymbol>';
}
...
method inclusive-or-operator:sym<|>($/) {
    make '<m:csymbol cd="bitwise1">or</m:csymbol>';
}
...
method logical-and-operator:sym<&&>($/) {
    make '<m:csymbol cd="logic1">and</m:csymbol>';
}
...
method logical-or-operator:sym<||>($/) {
    make '<m:csymbol cd="logic1">or</m:csymbol>';
}
...
method assignment-operator:sym<=>($/) {
    make '<prog1:assignment_expr/>';
}

method assignment-operator:sym<*=>($/) {
    make '<prog2:assignment_operator_expr/>'
       ~ '<arith1:times/>';
}
...
method expression($/) {
    # TODO: if elems > 1, then begin, otherwise...
    make (map {$_.ast}, @<assignment-expression>).join;
}
...
#    }
    ...
    my @inits = $<init-declarator-list>.ast;
    my $ast = map {$_($specs)}, @inits;
...
method declaration-specifiers($/) {
    my Str $ast = (map {$_.ast}, @<declaration-specifier>).join;
    if @<declaration-specifier>.elems > 1 {
        $ast = $AL_STAG
             ~ '<c89:direct_type/>'
             ~ $ast
             ~ $AL_ETAG;
...
method init-declarator($/) {
    ...
    my &decl = $<declarator> ?? $<declarator>.ast !! &identity;
    my $init = $<initializer> ?? $<initializer>.ast !! "";
    make -> $type {
        &decl($type, $init)
    }
...
method direct-declarator($/) {
    my $ast = $<direct-declarator-first>.ast;
...
method pointer:sym<pointer>($/) {
    my $mods = "";
    if @<type-qualifier-list>.elems > 0 {
        $mods = $MD_STAG ~ (map {$_.ast}, @<type-qualifier-list>).join ~ $MD_ETAG;
    }
    make -> $type {
        $AL_STAG
        ~ '<c89:pointer/>'
...
method function-definition:sym<std>($/) {
    ...
    my $type = get_declarator_name($<declarator>);
